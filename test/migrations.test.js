...
test('migration plan includes core tables by default', () => {
  assert.deepEqual(
    ...
    ['001_create_core_tables', '003_create_call_jobs', '004_create_app_settings']
  );
});
...
test('migration plan includes auth tables when auth is enabled', () => {
  assert.deepEqual(
    ...
    ['001_create_core_tables', '002_create_auth_tables', '003_create_call_jobs', '004_create_app_settings']
  );
});
